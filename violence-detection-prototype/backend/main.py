...
ALARM_DEVICE_ID = "eb799c3ee9c924555eareu"
ALARM_IP_ADDRESS = "192.168.1.10"
ALARM_LOCAL_KEY = ":s;oN`Yd45{*BzC6"
ALARM_DEVICE_VERSION = "3.5"
ALARM_DURATION = 3  # Duración de la alarma en segundos

# Inicializar dispositivo de alarma
try:
    alarm_device = tinytuya.Device(ALARM_DEVICE_ID, ALARM_IP_ADDRESS, ALARM_LOCAL_KEY, version=ALARM_DEVICE_VERSION)
    # Verificar conexión
    status = alarm_device.status()
    logger.info(f"Dispositivo de alarma conectado correctamente. Estado: {status}")
except Exception as e:
    logger.error(f"Error al conectar con el dispositivo de alarma: {str(e)}")
    alarm_device = None
...
last_alarm_trigger = 0  # Para evitar activar la alarma muy seguido
...
def trigger_alarm():
    ...
    # Evitar activar la alarma si ya se activó recientemente (dentro de 30 segundos)
    current_time = time.time()
    if current_time - last_alarm_trigger < 30:
        logger.info("Alarma no activada - activada recientemente")
        return
    
    if alarm_device is None:
        logger.warning("No se puede activar la alarma - dispositivo no disponible")
        return
    
    try:
        logger.info("🚨 ACTIVANDO ALARMA - VIOLENCIA DETECTADA 🚨")
        
        # Activar la sirena
        alarm_device.set_value(104, True)  # Enciende la sirena
        ...
        def deactivate_alarm():
            ...
            try:
                alarm_device.set_value(104, False)  # Apaga la sirena
                logger.info("🔇 Alarma desactivada")
            except Exception as e:
                logger.error(f"Error al desactivar la alarma: {str(e)}")
        
        # Ejecutar desactivación en hilo separado
        alarm_thread = threading.Thread(target=deactivate_alarm, daemon=True)
        alarm_thread.start()
        
    except Exception as e:
        logger.error(f"Error al activar la alarma: {str(e)}")
        ...
        try:
            ...
            if frame_count % STRIDE_FRAMES == 0 and len(frame_buffer) == CLIP_FRAMES:
                ...
                if violence_detected:
                    ...
                    # 🚨 ACTIVAR ALARMA CUANDO SE DETECTA VIOLENCIA 🚨
                    trigger_alarm()
...
class VideoTransformTrack(VideoStreamTrack):
    ...
    async def recv(self):
        ...
        if self.cap and self.cap.isOpened():
            ...
            # Indicador visual de alarma activada
            current_time = time.time()
            if current_time - last_alarm_trigger < ALARM_DURATION and last_alarm_trigger > 0:
                cv2.putText(frame, "🚨 ALARMA ACTIVADA 🚨", (10, 100), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 3)
